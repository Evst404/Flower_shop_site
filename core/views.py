...
def index(request):
    ...
    return render(request, 'index.html', {'recommended_bouquets': recommended_bouquets, 'consultation_form': consultation_form})
...
def card(request, bouquet_id):
    ...
    if request.method == 'POST' and 'consultation_submit' in request.POST:
        ...
        if consultation_form.is_valid():
            ...
            messages.error(request, 'Ошибка в заполнении формы.')
    return render(request, 'card.html', {'bouquet': bouquet, 'consultation_form': consultation_form})
...
def order(request, bouquet_id):
    ...
    if request.method == 'POST':
        ...
        if customer_form.is_valid() and order_form.is_valid():
            ...
            messages.error(request, 'Ошибка в заполнении формы. Проверьте данные.')
    else:
        customer_form = CustomerForm()
        order_form = OrderForm()
    return render(request, 'order.html', {'customer_form': customer_form, 'order_form': order_form, 'bouquet': bouquet})
...
def order_step(request, order_id):
    ...
    if request.method == 'POST':
        ...
        if payment_form.is_valid():
            ...
            send_mail(
                'Новый заказ',
                f'Букет: {order.bouquet.name}\nАдрес: {order.delivery_address}\nВремя: {order.delivery_time}\nКлиент: {order.customer.first_name} {order.customer.last_name}\nТелефон: {order.customer.phone_number}',
                settings.EMAIL_HOST_USER,
                ['courier@projectprimeflowershop.ru'],
                fail_silently=True,
            )
            messages.success(request, 'Оплата успешно обработана! Заказ принят.')
            return redirect('index')
        else:
            messages.error(request, 'Ошибка в данных оплаты. Проверьте введенные данные.')
    else:
        payment_form = PaymentForm()
    return render(request, 'order-step.html', {'payment_form': payment_form, 'order': order})
...
def quiz_step(request):
    if request.method == 'POST':
        ...
        if bouquet:
            ...
            messages.error(request, 'Нет подходящих букетов для выбранного события и бюджета.')
            return redirect('quiz_step')
